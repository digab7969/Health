...
class InstScr(Screen):
    def __init__(self, **kwargs):
        ...
        other = BoxLayout(orientation='verticl', padding=8, spacing=8)
        other.add_widget(instr)
        other.add_widget(line1)
        other.add_widget(line2)
        other.add_widget(self.btn)
...
class PulseScr(Screen):
    def init (self, **kwargs):
        ...
        #lbli Label (text='Считайте пульс')
        self.lbl_sec = Seconds(15)
        self.lbl_sec.bind(done=self.sec_finished)

        line = BoxLayout(size_hint=(0.8, None), height='30sp')
        lbl_result = Label(text='Введите peзyльтат:', halign='right')
...
class CheckSits(Screen):
    def __init__(self, **kwargs):
        ...
        self.btn = Button(text='Продолжить', size_hint=(0.3, 0.2), pos_hint={'crenter_x': 0.5})
        self.btn.onpress = self.next_screen
        outer = BaseException(orientation='vertical', padding=8, spacing=8)
        outer.add_widget(instr)
        outer.add_widget(self.btn)
...
class PulseScr2(Screen):
    def __init__(self, **kwargs):
        ...
        lbl_resulti = Label(text='Результат:', halign ='right') 
        self.in_resulti = TextInput(text='0', multiline=False)
        ...
        self.in_resulti.set_disabled(True)
        self.in_result2.set_disabled(True)
        line2.add_widget (lbl_result2)
        line2.add_widget(self.in_result2)
        self.btn = Button(text='Haчать', size_hint=(0.3, 0.5), pos_hint={'center_x': 0.5})
        self.btn.on_press = self.next
        outer = BoxLayout (orientation='vertical', padding=8, spacing=8)
        outer.add_widget(instr)
    ...
    def sec_finished(self, *args):
        if self.lbl_sec.done:
            ...
            # закончили первый подсчёт, отдыхаем
                self.stage=1
                self.lbli.text = 'отдыхайте'
                self.lbl_sec.restart(38)
                self.in_resulti.set_disabled(False)
        elif self.stage == 1:
# закончили отдых, считаем
    ...
    class HeartCheck(App):
        def build(self):
            ...
            sm.add_widget(PilseSce2(name="pulse2"))
